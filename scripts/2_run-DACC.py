...
os.chdir("C:/Users/Joe/github/em-methods-project/data-replication)

men_women = [[112,88],[100,100],[88,112]]

# change directory based on simulation type (1,2, or 3)
for sim_type in range(1,4):
    # number of men and women
    M = men_women[sim_type-1][0]
    W = men_women[sim_type-1][1]
    ...
    for sim_number in range(1,31):
        ...
        pref_M = pd.read_csv('simulated_data_' + str(sim_type) + '/pref_M_' + str(sim_number) + '.csv', delimiter=',')
        pref_W = pd.read_csv('simulated_data_' + str(sim_type) + '/pref_W_' + str(sim_number) + '.csv', delimiter=',')
