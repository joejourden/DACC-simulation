## number of men/women for each market type
men_women <- list(list(112,88),list(100,100),list(88,112))

## generate preference rankings
for (type in 1:3) {
  dataFolder <- paste0("./data-replication/simulated_data_",type)
  for (sim_number in 1:30){
    ...
    pref_M_dat <- data.frame(-pref_M) # women are negative integers now
    pref_W_dat <- data.frame(pref_W)
      
    write.table(pref_M_dat, file = paste(dataFolder,paste0("pref_M_",sim_number,".csv"),sep="/"), sep=",", row.names = FALSE, col.names = TRUE)
    write.table(pref_W_dat, file = paste(dataFolder,paste0("pref_W_",sim_number,".csv"),sep="/"), sep=",",  row.names = FALSE, col.names = TRUE)
  }
}