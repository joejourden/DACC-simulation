...
for (sim_number in 1:30){
  ...
  write.table(pref_M_dat, file = paste(dataFolder,"pref_M_",sim_number,".csv",sep=""), sep=",", row.names = FALSE, col.names = TRUE)
  write.table(pref_W_dat, file = paste(dataFolder,"pref_W_",sim_number,".csv",sep=""), sep=",",  row.names = FALSE, col.names = TRUE)
}
